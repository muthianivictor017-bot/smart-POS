...
function errorHandler(err,req,res,_next){const validation=err.name==='ZodError',status=err.status||(validation?400:500);if(status>=500)console.error(err);res.status(status).json({error:{code:err.code||(validation?'VALIDATION_ERROR':'INTERNAL_ERROR'),message:status>=500?'An unexpected error occurred':err.message,requestId:req.id}})}
